...
This repository contains a Java-based extension for Burp Suite using the burp.api.montoya API. The extension is designed to solve a specific lab involving blind OS command injection with out-of-band data exfiltration.
...
1. Open `LabSolver.java` and replace the URL at line `33` with your specific lab URL.
...
jar cvf 'Lab: Blind OS command injection with out-of-band data exfiltration.jar' -C build/ .
```
...
4. Select the JAR file `Lab: Blind OS command injection with out-of-band data exfiltration.jar`.
